...
void Figura::figuraToMatriz()
{
    ...
    case FIGURA_O:
        ...
        m_matriz[1][1] = 'X';
        break;
    default: // Si no ̩s una figura coneguda, la matriu segueix buida
        break;
    }
}
...
void Figura::dibuixa()
{
    for (int i = 0; i < MAX_ALCADA; i++)
        for (int j = 0; j < MAX_AMPLADA; j++)
            if (m_matriz[i][j] != ' ')
                dibuixaQuadrat(m_color, POS_X_TAULER + ((m_posicio[0] + j + 1) * MIDA_QUADRAT), POS_Y_TAULER + ((m_posicio[1] - 1 + i) * MIDA_QUADRAT));
}
